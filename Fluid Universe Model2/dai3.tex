...
% 优先加载中文支持宏包，解决编码与字体问题（XeLaTeX专用）
\usepackage{ctex}
% 基础数学与功能宏包
\usepackage{amsmath,amssymb,bm,graphicx,natbib,upgreek,orcidlink}
\usepackage[T1]{fontenc}

% 超链接设置（revtex4-2会自动加载hyperref，无需手动\usepackage）
\hypersetup{
    colorlinks=true,
    linkcolor=blue,
    citecolor=blue,
...
% 作者元数据（严格遵循revtex4-2规范，解决宏展开冲突）
\title{流体宇宙模型：宇宙学、引力与量子涌现的统一框架}
% 作者与ORCID绑定，避免单独命令导致解析错误
\author{董小勇\orcidlink{0009-0005-9433-4224}}
\affiliation{中国重庆市独立研究者}
\email{732414884@qq.com}
\homepage{https://github.com/FIUM132/Fluid-Universe-Model}
\date{\today}
...
\section*{数据可用性声明}
本研究的所有数据、代码与推导结果，均已开源至GitHub仓库（https://github.com/FIUM132/Fluid-Universe-Model），可完全复现本文的所有结果。所有观测数据均来自公开的天文数据库，具体引用见正文参考文献。
